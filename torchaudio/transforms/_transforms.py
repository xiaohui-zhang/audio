...
class SpecAugment(torch.nn.Module):
    ...
    Args:
        ...
        p (float, optional): maximum proportion of time steps that can be masked.
            Must be within range [0.0, 1.0]. (Default: 1.0)
        zero_masking (bool): If True, use 0 as the mask value with 0, else use mean of the input tensor.
    """
    __constants__ = ["mask_param", "axis", "iid_masks", "p"]

    def __init__(self, n_time_masks: int, time_mask_param: int, n_freq_masks: int, freq_mask_param: int, iid_masks: bool = True, p: float = 1.0, zero_masking: bool = False) -> None:
